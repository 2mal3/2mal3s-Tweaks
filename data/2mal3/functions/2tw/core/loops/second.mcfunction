...
execute if score $nerfed_elytra 2tw.config matches 1 if entity @r[predicate=2mal3:2tw/in_end,gamemode=!spectator] as @e[type=minecraft:item_frame,tag=!2tw.processed,tag=!global.ignore,predicate=2mal3:2tw/tweaks/nerfed_elytra,predicate=2mal3:2tw/in_end] run function 2mal3:2tw/tweaks/nerfed_elytra
...
execute if score $better_end_crystals 2tw.config matches 1 as @e[type=minecraft:ender_dragon,tag=!2tw.processed] as @e[type=minecraft:end_crystal,predicate=2mal3:2tw/in_end,tag=!global.ignore] at @s if block ~ ~-1 ~ minecraft:bedrock if block ~ ~2 ~ minecraft:iron_bars if block ~2 ~ ~ minecraft:iron_bars if block ~-2 ~ ~ minecraft:iron_bars if block ~ ~ ~2 minecraft:iron_bars if block ~ ~ ~-2 minecraft:iron_bars run function 2mal3:2tw/tweaks/better_end_crystals
 
# Prevents creeper from doing blok damage
execute if score $anti_creeper_grief 2tw.config matches 1 as @e[type=minecraft:creeper,tag=!2tw.processed,tag=!global.ignore] run function 2mal3:2tw/tweaks/anti_creeper_grief
