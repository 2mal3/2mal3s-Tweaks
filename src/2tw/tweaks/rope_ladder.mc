...
function place {
  ...
  execute if data storage 2tw:data gamerules{rope_ladder: 1b} anchored eyes run {
...
function set_blocks {
  execute if block ~ ~ ~ #2tw:utilities/air run {
    ...
    execute store success score .temp_0 2tw.data run clear @s minecraft:ladder 1

    execute if score .temp_0 2tw.data matches 1 run {
      clone ~ ~1 ~ ~ ~1 ~ ~ ~ ~
...
## Breaking the ladder
function loop {
  execute as @a[scores={2tw.mined_ladder=1..,2tw.sneaking=1..}] at @s at @e[type=minecraft:item, nbt={Item: {id: "minecraft:ladder", Count: 1b}, Age: 0s}, limit=1, sort=nearest] run {
  ...
  scoreboard players reset @a[scores={2tw.mined_ladder=1..}] 2tw.mined_ladder
}
